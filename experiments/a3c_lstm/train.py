...
def train(
    ...
    while total_episodes < max_episodes:
        ...
        for step in range(num_bootstrap_steps):
            ...
            # Penalize for longer trajectories
            #reward -= episode_length / max_episode_len
        ...
        for i in reversed(range(len(rewards))):
            ...
            policy_loss = policy_loss - \
                log_probs[i] * torch.tensor(advantage)# - 0.01 * entropies[i]
    ...
    while waited_episodes <= max_episodes:
        ...
        if waited_episodes % test_every == 0:
            ...
            for instruction in instructions:
                ...
                #print("Instruction: {}".format(instruction_raw))
                #print("Instruction indices: {}".format(instruction_idx))

                # Get the instruction representation
                with torch.no_grad():
                    encoder_hidden = torch.zeros(1, 1, shared_model.gru_hidden_size)
                    for i in range(instruction_idx.data.size(1)):
                        word_embedding    = shared_model.embedding(instruction_idx[0, i]).view(1, 1, -1)
                        _, encoder_hidden = shared_model.gru(word_embedding, encoder_hidden)
                    x_instr_rep = encoder_hidden.view(encoder_hidden.size(1), -1)
                    embeds.append(x_instr_rep)

            for embed_ind in range(len(embeds)):
                for embed1_ind in range(embed_ind + 1, len(embeds)):
                    embed  = embeds[embed_ind]
                    embed1 = embeds[embed1_ind]

                    dist = F.pairwise_distance(embed, embed1)
                    cos  = F.cosine_similarity(embed, embed1)
                    print("Embed {} and {} dist: {}".format(embed_ind, embed1_ind, dist), flush=True)
                    print("Embed {} and {} cos: {}".format(embed_ind, embed1_ind, cos), flush=True)
