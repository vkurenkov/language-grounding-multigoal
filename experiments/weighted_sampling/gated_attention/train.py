...
# Target environment
from experiments.weighted_sampling.gated_attention.parameters import env_definition

# Instructions
from experiments.weighted_sampling.gated_attention.parameters import instructions
from experiments.weighted_sampling.gated_attention.parameters import instructions_level
from experiments.weighted_sampling.gated_attention.parameters import tokenizer

# Agent's training and testing parameters
from experiments.weighted_sampling.gated_attention.parameters import train_parameters
from experiments.weighted_sampling.gated_attention.parameters import test_parameters
from experiments.weighted_sampling.gated_attention.parameters import experiment_folder
from experiments.weighted_sampling.gated_attention.parameters import TEST_MODE_STOCHASTIC
from experiments.weighted_sampling.gated_attention.parameters import TEST_MODE_DETERMINISTIC

# Computational model
from experiments.weighted_sampling.gated_attention.model import A3C_LSTM_GA
...
def sample_instruction(instructions: List[NaturalLanguageInstruction], weighted=False) -> NaturalLanguageInstruction:
    if weighted:
        ind = np.random.choice(len(instructions), p=instruction_weights)
    else:
...
def train(
    ...
    instruction     = sample_instruction(instructions, weighted=True)
    ...
    while total_episodes < max_episodes:
        ...
        if not done:
            ...
            print("should never be here, mkay?")
...
def test(
    ...
    succ_rate_run_avg = defaultdict(float)
    ...
    while waited_episodes <= max_episodes:
        ...
        if waited_episodes % test_every == 0:
            ...
            for instruction in instructions:
                ...
                # Update weights
                succ_rate_run_avg[instruction_raw] = succ_rate_run_avg[instruction_raw] * (1-sampling_smooth) + mean_suc_rate * sampling_smooth
                new_instruction_weights.append(1.0 - succ_rate_run_avg[instruction_raw]) # The less successful the more we want to sample it
            ...
            # Reweight instructions
            weights_sum             = sum(new_instruction_weights)
            new_instruction_weights = [weight / weights_sum for weight in new_instruction_weights]
            for i in range(len(instruction_weights)):
                instruction_weights[i] = new_instruction_weights[i]

            logger.add_histogram("Instrution/SampleWeights", np.array(new_instruction_weights), waited_episodes)
...
agent.share_memory()
instruction_weights = mp.Array(ctypes.c_double, [1.0 / len(instructions) for _ in instructions])
...
test_args = (
    ...
    train_parameters["sampling_smooth"],
