...
def prepare_model_input(observation, instruction_indices) -> Tuple[torch.Tensor, torch.Tensor]:
	return (torch.tensor(observation).unsqueeze(0).to(device), torch.tensor(instruction_indices, dtype=torch.int64).to(device))
