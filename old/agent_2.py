...
TARGET_SWITCH_FRAMES = 50000

NUM_TRIALS = 20
seed = -1

for trial in range(NUM_TRIALS):
    with Session("agent-2-trial-" + str(trial)).start() as sess:
        sess.switch_group()
        ...
        while num_frames < FRAMES:
            ...
            # Swap target and behaviour networks
            if num_frames - last_switch >= TARGET_SWITCH_FRAMES:
                sess.log("Switch copy target to behaviour network: " + str(num_frames))
                sess.log("\n")
                behaviour_q = copy.deepcopy(target_q)
                last_switch = num_frames
